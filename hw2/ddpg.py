...
class Actor(nn.Module):
    ...
    def forward(self, inputs):
        ...
        x = self.net(inputs)
        x = x * 4.0 - 2.0  # Linear mapping to [-2.0, 2.0]
        return x
        ########## END OF YOUR CODE ##########
...
class Critic(nn.Module):
    ...
    def __init__(self, hidden_size, num_inputs, action_space):
        ...
        self.state_net = nn.Sequential(nn.Linear(num_inputs, hidden_size))
...
class DDPG(object):
    ...
    def save_model(self,
        ...
        if actor_path is None:
            actor_path = f'pre_trained/ddpg_actor_{env_name}_{timestamp}_{suffix}'
        if critic_path is None:
            critic_path = f'pre_trained/ddpg_critic_{env_name}_{timestamp}_{suffix}'
...
def train(gamma=0.95, tau=0.005):
    num_episodes = 500
    hidden_size = 128
    noise_scale = 0.4
    replay_size = 50000
...
if __name__ == '__main__':
    ...
    writer = SummaryWriter(f'logs/{ENV_NAME}/gamma_{GAMMA}_tau_{TAU}')
    train(gamma=GAMMA, tau=TAU)
    writer.close()
